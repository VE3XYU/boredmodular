...
**Next concrete step for the next agent:** Phase 5 (inline ports next to their controls). It is now the only remaining phase from the original implementation slate — it restructures port layout system-wide and is the highest-risk piece on cables.
...
- ~~**Phase 8** — sub-section frames + Osc-family labels.~~ Unit 1 shipped in PR #71 (2026-05-26): optional `paramRows[i].label` field; renderer wraps labelled rows in a recessed-well rectangle with an uppercase header; DrumSynth's 4 rows carry MASTER / SLAVE / FILTER / BEND per `MODULE_LAYOUTS.md` §2.16. Unit 2 shipped in PR #73 (2026-05-28): MasterOsc (`PITCH`), OscA (`PITCH` / `PW` / `FM`), OscB / OscC (`PITCH` / `FM`). Unlabelled rows render byte-identical. Sub-plan `docs/plans/2026-05-26-001-feat-ui-module-subsections-plan.md` closed 2026-05-28.
