...
Spec location: `BORED_MODULAR_DESIGN.md` lines 5-39.

## 0.1 Signal Types (`BORED_MODULAR_DESIGN.md:7-11`)
...
- **Minor** (`BORED_MODULAR_DESIGN.md:8` — "Audio/Control (Bipolar)") — combines the PDF's separately-named "Audio" and "Control (Bipolar)" categories into a single label. Inline PDF usage shows the manual treats `Bipolar` as a single signal-type label applied to both audio outputs and bipolar control outputs, so combining them in the spec is reasonable, but the spec's compound name is not the manual's. Resolution: rename to `Bipolar` (drop the `Audio/Control` prefix) to match the PDF's inline usage of `Signal: Bipolar.`

## 0.2 Connector Colors (`BORED_MODULAR_DESIGN.md:13-17`)
...
## 0.3 Modulation Input Attenuator Types (`BORED_MODULAR_DESIGN.md:19-22`)
...
## 0.4 Module Limits (`BORED_MODULAR_DESIGN.md:24-27`)

- **Verified** — `BORED_MODULAR_DESIGN.md:25` ("Max 254 modules per patch (127 poly, 127 common)") matches PDF p.1: "you could use up to 254 modules in each patch, 127 in the Poly Voice Area and 127 in the Common Voice Area."
- **Verified** — `BORED_MODULAR_DESIGN.md:26` ("Keyboard, KeyboardPatch, MIDIGlobal, AudioIn: max one per voice area") matches PDF p.1: "These modules can only be used twice in a patch, one in the Poly Voice Area and one in the Common Voice Area." (Spec wording is equivalent — one per voice area, two voice areas → two per patch.)
- **Verified** — `BORED_MODULAR_DESIGN.md:27` ("PolyAreaIn: one per patch, common voice area only") matches PDF p.1: "The 'Poly Area In' module can only be used once in a patch, in the Common Voice Area."
- **Minor** (`BORED_MODULAR_DESIGN.md:26` — name `KeyboardPatch`) — PDF uses the two-word form `Keyboard Patch` (e.g. p.2 "KeyboardPatch" panel header but body text "Keyboard Patch module"). Spec contracts to one word, matching the panel header. Acceptable.

## 0.5 Morphing (`BORED_MODULAR_DESIGN.md:31-35`)
...
## 0.6 Variations (`BORED_MODULAR_DESIGN.md:37-39`)
...
Spec: `BORED_MODULAR_DESIGN.md:45-50`, `MODULE_LAYOUTS.md:24-27`.
PDF: pp.1-2.
...
Spec: `BORED_MODULAR_DESIGN.md:52-57`, `MODULE_LAYOUTS.md:29-31`.
PDF: p.2.
...
- **Minor** (`BORED_MODULAR_DESIGN.md:52`, `MODULE_LAYOUTS.md:29`) — module name `KeyboardPatch` is one word in spec; PDF body text uses `Keyboard Patch` (two words) but the panel header is `KeyboardPat..1` (truncated single-word). The single-word spec name is consistent with the panel header. Resolution: leave as is, or align to PDF body wording.

## 1.3 MIDIGlobal

Spec: `BORED_MODULAR_DESIGN.md:59-63`, `MODULE_LAYOUTS.md:33-35`.
PDF: p.3.
...
Spec: `BORED_MODULAR_DESIGN.md:65-68`, `MODULE_LAYOUTS.md:37-40`.
PDF: p.4.
...
Spec: `BORED_MODULAR_DESIGN.md:70-74`, `MODULE_LAYOUTS.md:42-44`.
PDF: p.4.
...
Spec: `BORED_MODULAR_DESIGN.md:76-81`, `MODULE_LAYOUTS.md:46-48`.
PDF: p.5.
...
Spec: `BORED_MODULAR_DESIGN.md:83-88`, `MODULE_LAYOUTS.md:50-52`.
PDF: p.5.
...
Spec: `BORED_MODULAR_DESIGN.md:90-93`, `MODULE_LAYOUTS.md:54-56`.
PDF: p.6.

- **Verified** — four Mix bus inputs (Red), Level knob. Layout matches.
- **Minor** (`BORED_MODULAR_DESIGN.md:90`) — spec says "Routes four separate signals to individual mix buses"; PDF says "routes four separate signals to one mix bus each." Equivalent meaning.

## 1.9 NoteDetect

Spec: `BORED_MODULAR_DESIGN.md:95-100`, `MODULE_LAYOUTS.md:58-60`.
PDF: p.6.
...
Spec: `BORED_MODULAR_DESIGN.md:102-110`, `MODULE_LAYOUTS.md:62-65`.
PDF: pp.6-7.
...
Spec: `BORED_MODULAR_DESIGN.md:127-141`, `MODULE_LAYOUTS.md:77-81`.
PDF: pp.8-10.
...
- **Minor** (`BORED_MODULAR_DESIGN.md:129`) — spec describes the first waveform as "Pulse"; PDF p.8 says "Square/Pulse". Same minor naming difference exists in `OscB:144`, `OscSlvA:187`. Resolution: rename to `Square/Pulse` for consistency with PDF.

## 2.3 OscB

Spec: `BORED_MODULAR_DESIGN.md:142-151`, `MODULE_LAYOUTS.md:83-87`.
PDF: pp.10-11.
...
- **Minor** (`BORED_MODULAR_DESIGN.md:144`) — same `Pulse` vs `Square/Pulse` naming as OscA.

## 2.4 OscC

Spec: `BORED_MODULAR_DESIGN.md:152-162`, `MODULE_LAYOUTS.md:89-93`.
PDF: pp.11-12.
...
Spec: `BORED_MODULAR_DESIGN.md:163-173`, `MODULE_LAYOUTS.md:95-99`.
PDF: pp.12-14.
...
- **Critical** (`BORED_MODULAR_DESIGN.md:167`) — `**Coarse/Fine/KBT:** Standard.` is ambiguous and likely misleading. PDF p.14 describes SpectralOsc's KBT as binary ("If KBT is activated... If KBT is not activated"), same as MasterOsc/OscC, NOT the OscA-style 0.0-2.0 knob implied by "Standard". Resolution: replace with `**Coarse/Fine (Knobs):** Standard. **KBT (Button):** On/Off.`
- **Critical** (`MODULE_LAYOUTS.md:97`) — layout lists `[KBT Knob]`; should be `[KBT Button]` to match PDF panel.

## 2.6 FormantOsc

Spec: `BORED_MODULAR_DESIGN.md:175-183`, `MODULE_LAYOUTS.md:101-105`.
PDF: pp.14-15.
...
- **Minor** (`BORED_MODULAR_DESIGN.md:187`) — same `Pulse` vs `Square/Pulse` waveform naming as OscA/OscB.

## 2.8 OscSlvB

Spec: `BORED_MODULAR_DESIGN.md:199-206`, `MODULE_LAYOUTS.md:113-117`.
PDF: p.16.
...
Spec: `BORED_MODULAR_DESIGN.md:208-214`, `MODULE_LAYOUTS.md:119-123`.
PDF: p.17.
...
Spec: `BORED_MODULAR_DESIGN.md:216-222`, `MODULE_LAYOUTS.md:125-127`.
PDF: p.18.
...
Spec: `BORED_MODULAR_DESIGN.md:224-231`, `MODULE_LAYOUTS.md:129-133`.
PDF: p.19.
...
Spec: `BORED_MODULAR_DESIGN.md:233-244`, `MODULE_LAYOUTS.md:135-140`.
PDF: p.20.
...
Spec: `BORED_MODULAR_DESIGN.md:246-254`, `MODULE_LAYOUTS.md:142-146`.
PDF: pp.21-22.
...
Spec: `BORED_MODULAR_DESIGN.md:256-259`, `MODULE_LAYOUTS.md:148-150`.
PDF: p.22.
...
Spec: `BORED_MODULAR_DESIGN.md:261-272`, `MODULE_LAYOUTS.md:152-156`.
PDF: p.23.
...
Spec: `BORED_MODULAR_DESIGN.md:274-296`, `MODULE_LAYOUTS.md:158-169`.
PDF: pp.24-25.
...
Spec: `BORED_MODULAR_DESIGN.md:304-316`, `MODULE_LAYOUTS.md:175-177`.
PDF: pp.26-27.
...
Spec: `BORED_MODULAR_DESIGN.md:318-326`, `MODULE_LAYOUTS.md:179-181`.
PDF: pp.27-29.
...
Spec: `BORED_MODULAR_DESIGN.md:327-336`, `MODULE_LAYOUTS.md:183-185`.
PDF: p.29.
...
Spec: `BORED_MODULAR_DESIGN.md:338-347`, `MODULE_LAYOUTS.md:187-189`.
PDF: p.30.
...
Spec: `BORED_MODULAR_DESIGN.md:349-353`, `MODULE_LAYOUTS.md:191-193`.
PDF: p.31.

- **Verified** — Sawtooth slave with Mst (Gray), Display, Rate knob (0.025-38.05× master), Out (Blue).
- **Minor** (`BORED_MODULAR_DESIGN.md:353`) — Out description "Bipolar" omits the LED indicator. PDF p.31 says: "The LED above the output shows an approximation of the current LFO rate. Signal: Bipolar." Layout already includes "+ LED" but body is inconsistent. Resolution: append "LED indicates rate." to the Out bullet, matching LFOA and LFOSlvA style.

## 3.6 LFOSlvC

Spec: `BORED_MODULAR_DESIGN.md:355-359`, `MODULE_LAYOUTS.md:195-197`.
PDF: pp.31-32.

- **Verified** — Sine slave; same controls as LFOSlvB.
- **Minor** (`BORED_MODULAR_DESIGN.md:359`) — same missing-LED note as LFOSlvB.

## 3.7 LFOSlvD

Spec: `BORED_MODULAR_DESIGN.md:361-365`, `MODULE_LAYOUTS.md:199-201`.
PDF: p.32.

- **Verified** — Square slave; same controls.
- **Minor** (`BORED_MODULAR_DESIGN.md:365`) — same missing-LED note.

## 3.8 LFOSlvE

Spec: `BORED_MODULAR_DESIGN.md:367-371`, `MODULE_LAYOUTS.md:203-205`.
PDF: pp.32-33.

- **Verified** — Triangle slave; same controls.
- **Minor** (`BORED_MODULAR_DESIGN.md:371`) — same missing-LED note.

## 3.9 ClkGen

Spec: `BORED_MODULAR_DESIGN.md:373-381`, `MODULE_LAYOUTS.md:207-209`.
PDF: p.34.
...
Spec: `BORED_MODULAR_DESIGN.md:383-388`, `MODULE_LAYOUTS.md:211-213`.
PDF: p.35.
...
Spec: `BORED_MODULAR_DESIGN.md:390-394`, `MODULE_LAYOUTS.md:215-217`.
PDF: p.35.
...
Spec: `BORED_MODULAR_DESIGN.md:396-400`, `MODULE_LAYOUTS.md:219-221`.
PDF: p.36.
...
Spec: `BORED_MODULAR_DESIGN.md:402-405`, `MODULE_LAYOUTS.md:223-225`.
PDF: p.36.
...
Spec: `BORED_MODULAR_DESIGN.md:407-417`, `MODULE_LAYOUTS.md:227-230`.
PDF: p.37.
...
Spec: `BORED_MODULAR_DESIGN.md:424-438`, `MODULE_LAYOUTS.md:236-240`.
PDF: pp.38-39.
...
Spec: `BORED_MODULAR_DESIGN.md:440-449`, `MODULE_LAYOUTS.md:242-244`.
PDF: pp.40-41.
...
Spec: `BORED_MODULAR_DESIGN.md:451-461`, `MODULE_LAYOUTS.md:246-251`.
PDF: pp.41-42.
...
Spec: `BORED_MODULAR_DESIGN.md:463-473`, `MODULE_LAYOUTS.md:253-258`.
PDF: pp.42-44.
...
Spec: `BORED_MODULAR_DESIGN.md:475-486`, `MODULE_LAYOUTS.md:260-265`.
PDF: pp.44-45.
...
Spec: `BORED_MODULAR_DESIGN.md:488-493`, `MODULE_LAYOUTS.md:267-269`.
PDF: pp.45-46.
...
Spec: `BORED_MODULAR_DESIGN.md:499-503`, `MODULE_LAYOUTS.md:275-277`.
PDF: p.46.
...
Spec: `BORED_MODULAR_DESIGN.md:505-509`, `MODULE_LAYOUTS.md:279-281`.
PDF: p.46.
...
Spec: `BORED_MODULAR_DESIGN.md:511-519`, `MODULE_LAYOUTS.md:283-285`.
PDF: p.47.
...
Spec: `BORED_MODULAR_DESIGN.md:521-530`, `MODULE_LAYOUTS.md:287-291`.
PDF: p.48.
...
Spec: `BORED_MODULAR_DESIGN.md:532-545`, `MODULE_LAYOUTS.md:293-297`.
PDF: pp.49-50.
...
Spec: `BORED_MODULAR_DESIGN.md:547-557`, `MODULE_LAYOUTS.md:299-303`.
PDF: pp.50-51.
...
Spec: `BORED_MODULAR_DESIGN.md:559-568`, `MODULE_LAYOUTS.md:305-309`.
PDF: pp.51-52.
...
Spec: `BORED_MODULAR_DESIGN.md:570-580`, `MODULE_LAYOUTS.md:311-317`.
PDF: pp.52-53.
...
- **Critical** (`BORED_MODULAR_DESIGN.md:576`) — spec says `Shift buttons (+1 to +15)` for the preset shift range. PDF panel p.52 shows preset shift buttons as `-2, -1, 0, +1, +2, Inv, Rnd` (a small symmetric range, not +1 to +15). PDF p.53 body text describes them as "all Synthesis bands the number of steps indicated on the buttons" — the actual button labels are -2 through +2. Resolution: replace `+1 to +15` with `-2 to +2` (and update the surrounding wording: "Shift buttons (-2 to +2 steps)").

## 5.9 FilterBank

Spec: `BORED_MODULAR_DESIGN.md:582-588`, `MODULE_LAYOUTS.md:319-322`.
PDF: pp.52-53.
...
Spec: `BORED_MODULAR_DESIGN.md:590-598`, `MODULE_LAYOUTS.md:324-327`.
PDF: pp.53-54.
...
Spec: `BORED_MODULAR_DESIGN.md:600-608`, `MODULE_LAYOUTS.md:329-332`.
PDF: p.54.
...
Spec: `BORED_MODULAR_DESIGN.md:616-619`, `MODULE_LAYOUTS.md:338-341`.
PDF: p.55.
...
Spec: `BORED_MODULAR_DESIGN.md:621-625`, `MODULE_LAYOUTS.md:343-346`.
PDF: p.55.
...
Spec: `BORED_MODULAR_DESIGN.md:627-633`, `MODULE_LAYOUTS.md:348-350`.
PDF: p.56.
...
Spec: `BORED_MODULAR_DESIGN.md:635-640`, `MODULE_LAYOUTS.md:352-354`.
PDF: pp.57-58.
...
Spec: `BORED_MODULAR_DESIGN.md:642-648`, `MODULE_LAYOUTS.md:356-358`.
PDF: p.58.
...
Spec: `BORED_MODULAR_DESIGN.md:650-654`, `MODULE_LAYOUTS.md:360-362`.
PDF: p.58.
...
Spec: `BORED_MODULAR_DESIGN.md:656-660`, `MODULE_LAYOUTS.md:364-366`.
PDF: pp.58-59.
...
Spec: `BORED_MODULAR_DESIGN.md:662-667`, `MODULE_LAYOUTS.md:368-370`.
PDF: p.59.
...
Spec: `BORED_MODULAR_DESIGN.md:669-674`, `MODULE_LAYOUTS.md:372-374`.
PDF: p.59.
...
Spec: `BORED_MODULAR_DESIGN.md:676-680`, `MODULE_LAYOUTS.md:376-378`.
PDF: p.60.
...
Spec: `BORED_MODULAR_DESIGN.md:682-687`, `MODULE_LAYOUTS.md:380-383`.
PDF: p.60.
...
Spec: `BORED_MODULAR_DESIGN.md:689-694`, `MODULE_LAYOUTS.md:385-389`.
PDF: p.61.
...
Spec: `BORED_MODULAR_DESIGN.md:696-700`, `MODULE_LAYOUTS.md:391-393`.
PDF: p.61.
...
Spec: `BORED_MODULAR_DESIGN.md:706-713`, `MODULE_LAYOUTS.md:399-401`.
PDF: p.62.
...
Spec: `BORED_MODULAR_DESIGN.md:715-721`, `MODULE_LAYOUTS.md:403-405`.
PDF: pp.62-63.
...
Spec: `BORED_MODULAR_DESIGN.md:723-729`, `MODULE_LAYOUTS.md:407-409`.
PDF: p.63.
...
Spec: `BORED_MODULAR_DESIGN.md:731-735`, `MODULE_LAYOUTS.md:411-413`.
PDF: p.64.
...
Spec: `BORED_MODULAR_DESIGN.md:737-743`, `MODULE_LAYOUTS.md:415-417`.
PDF: p.64.
...
Spec: `BORED_MODULAR_DESIGN.md:745-750`, `MODULE_LAYOUTS.md:419-421`.
PDF: p.65.
...
Spec: `BORED_MODULAR_DESIGN.md:752-755`, `MODULE_LAYOUTS.md:423-425`.
PDF: p.65.
...
Spec: `BORED_MODULAR_DESIGN.md:757-763`, `MODULE_LAYOUTS.md:427-429`.
PDF: p.66.
...
Spec: `BORED_MODULAR_DESIGN.md:765-779`, `MODULE_LAYOUTS.md:431-435`.
PDF: pp.66-67.
...
Spec: `BORED_MODULAR_DESIGN.md:781-789`, `MODULE_LAYOUTS.md:437-439`.
PDF: p.68.
...
Spec: `BORED_MODULAR_DESIGN.md:791-796`, `MODULE_LAYOUTS.md:441-443`.
PDF: pp.68-69.
...
Spec: `BORED_MODULAR_DESIGN.md:798-812`, `MODULE_LAYOUTS.md:445-449`.
PDF: pp.70-71.
...
Spec: `BORED_MODULAR_DESIGN.md:814-828`, `MODULE_LAYOUTS.md:451-455`.
PDF: pp.71-72.
...
Spec: `BORED_MODULAR_DESIGN.md:830-836`, `MODULE_LAYOUTS.md:457-459`.
PDF: p.72.
...
Spec: `BORED_MODULAR_DESIGN.md:838-846`, `MODULE_LAYOUTS.md:461-465`.
PDF: p.75.
...
Spec: `BORED_MODULAR_DESIGN.md:852-856`, `MODULE_LAYOUTS.md:471-473`.
PDF: p.76.
...
Spec: `BORED_MODULAR_DESIGN.md:858-862`, `MODULE_LAYOUTS.md:475-477`.
PDF: p.76.
...
Spec: `BORED_MODULAR_DESIGN.md:864-868`, `MODULE_LAYOUTS.md:479-481`.
PDF: p.77.
...
Spec: `BORED_MODULAR_DESIGN.md:870-876`, `MODULE_LAYOUTS.md:483-485`.
PDF: p.77.
...
Spec: `BORED_MODULAR_DESIGN.md:879-883`, `MODULE_LAYOUTS.md:487-489`.
PDF: p.78.
...
Spec: `BORED_MODULAR_DESIGN.md:885-890`, `MODULE_LAYOUTS.md:491-493`.
PDF: p.78.
...
Spec: `BORED_MODULAR_DESIGN.md:892-898`, `MODULE_LAYOUTS.md:495-498`.
PDF: pp.79-80.
...
Spec: `BORED_MODULAR_DESIGN.md:900-904`, `MODULE_LAYOUTS.md:500-502`.
PDF: p.80.
...
Spec: `BORED_MODULAR_DESIGN.md:906-911`, `MODULE_LAYOUTS.md:504-506`.
PDF: p.81.
...
Spec: `BORED_MODULAR_DESIGN.md:913-922`, `MODULE_LAYOUTS.md:508-512`.
PDF: pp.81-82.
...
Spec: `BORED_MODULAR_DESIGN.md:930-934`, `MODULE_LAYOUTS.md:518-520`.
PDF: p.83.
...
Spec: `BORED_MODULAR_DESIGN.md:936-940`, `MODULE_LAYOUTS.md:522-524`.
PDF: p.83.
...
Spec: `BORED_MODULAR_DESIGN.md:942-946`, `MODULE_LAYOUTS.md:526-528`.
PDF: p.84.
...
Spec: `BORED_MODULAR_DESIGN.md:948-952`, `MODULE_LAYOUTS.md:530-532`.
PDF: p.84.
...
Spec: `BORED_MODULAR_DESIGN.md:954-958`, `MODULE_LAYOUTS.md:534-536`.
PDF: p.85.
...
Spec: `BORED_MODULAR_DESIGN.md:960-967`, `MODULE_LAYOUTS.md:538-540`.
PDF: p.85.
...
Spec: `BORED_MODULAR_DESIGN.md:969-973`, `MODULE_LAYOUTS.md:542-544`.
PDF: p.86.
...
Spec: `BORED_MODULAR_DESIGN.md:975-978`, `MODULE_LAYOUTS.md:546-548`.
PDF: p.86.
...
Spec: `BORED_MODULAR_DESIGN.md:980-985`, `MODULE_LAYOUTS.md:550-552`.
PDF: p.87.
...
Spec: `BORED_MODULAR_DESIGN.md:987-991`, `MODULE_LAYOUTS.md:554-556`.
PDF: p.87.
...
Spec: `BORED_MODULAR_DESIGN.md:1002-1013`, `MODULE_LAYOUTS.md:562-567`.
PDF: pp.88-89.
...
Spec: `BORED_MODULAR_DESIGN.md:1015-1027`, `MODULE_LAYOUTS.md:569-574`.
PDF: pp.89-90.
...
Spec: `BORED_MODULAR_DESIGN.md:1029-1043`, `MODULE_LAYOUTS.md:576-581`.
PDF: pp.90-91.
...
Spec: `BORED_MODULAR_DESIGN.md:1045-1053`, `MODULE_LAYOUTS.md:583-588`.
PDF: pp.92-93.
...
1. **MasterOsc KBT** (`BORED_MODULAR_DESIGN.md:122`, `MODULE_LAYOUTS.md:73`) — described as a 0-2.0 knob; PDF p.7 confirms it's a binary on/off button.
2. **SpectralOsc KBT** (`BORED_MODULAR_DESIGN.md:167`, `MODULE_LAYOUTS.md:97`) — `Coarse/Fine/KBT: Standard` is ambiguous and implies OscA-style knob; PDF p.14 confirms it's a binary on/off button.
3. **Vocoder preset shift buttons** (`BORED_MODULAR_DESIGN.md:576`) — spec lists `+1 to +15`; PDF p.52 panel shows -2, -1, 0, +1, +2 plus Inv and Rnd.
