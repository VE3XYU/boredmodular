...
**Status: 2026-06-02.** Post-review fix batch (surfaced by a 12-lens code review of `src/`) merged to `main`. **FilterC-F5 / FilterE-F5 (GC gain compensation) RESOLVED** (PR #76): GC now cancels the biquad's resonance peak boost so output level holds roughly constant as resonance rises — the behaviour the manual documents. Per-type and dB-aware: Web Audio parameterises LP/HP `Q` in dB, so `Qlin = 10^(q/20)` and the resonant peak ≈ `Qlin`; makeup `√(1−1/(4·Qlin²))/Qlin` is applied to LP/HP only (BP/notch are 0 dB-normalised → no makeup), squared for FilterE's 24 dB cascade. Validated to 0.00 dB residual against the Chromium biquad coefficient math. The exact hardware GC curve is proprietary/unpublished (pch2csd left it unimplemented), so this is a DSP-grounded reconstruction of the documented intent (playbook §2). Two **post-audit fidelity fixes** also shipped — neither was an original audit finding, because the Batch 3 sweep checked params/ranges/IO, not runtime state: unmastered **slave oscillators now free-run at E4 (329.6 Hz)** with tuning knobs applied, per `BORED_MODULAR_DESIGN.md:139` (PR #78; impl had frozen at a hardcoded 220 Hz with inert knobs, and slaves stayed frozen on master disconnect/removal), and **FormantOsc vowel change now honours live timbre** within the `FmtOsc-F1` keep-as-divergence model (PR #79). Non-fidelity robustness/cleanup also merged the same day (patch-load hardening, input/audio-context lifecycle, dead-code removal): PRs #75, #77. `XFade-F5` (FadeMod dead-end) **RESOLVED** (this branch): the crossfade's labelled `FadeMod` input was wired to a `fadeMod` GainNode that was created (gain 0) and accepted as the input target but never connected onward, so modulation was inert. `_createXFade` now passes the CV through at unity and fans it out to the gain pair the way the fade knob drives them — `fadeMod → gainB.gain` (+) and `fadeMod → (−1 inverter) → gainA.gain` (−) — so a CV sweeps the crossfade bipolarly around the knob centre. This is the inverse-gain-pair option named in the original disposition; no worklet/scheduler, no `MODULE_DEFS`/port/param change, patch-load-safe. Verified in Chromium: an OfflineAudioContext A/B of the exact graph (pre-fix output flat at 0.5 across FadeMod ∈ {0, ±0.4}; post-fix 0.5 → 0.1 / 0.9) and a live RMS sweep (0.01–0.44 at the LFO rate) loading `examples/xfade-fademod-test.json`. The `FadeMod` Type I input attenuator (XFade-F3) stays folded to systemic finding S2; the remaining open Critical items are all systemic (S1–S4), blocked on their own brainstorm + plan by design.
...
- **FilterC-F5 — GC behaviour not actually implemented.** Impl `setParam` has no branch that consumes `gainComp` (`AudioEngine.js:1984-1991` only syncs freq and resonance). The param is stored but doesn't reduce gain at high resonance. Severity: Critical (the labelled control does nothing). Disposition: `fix-toward-spec — RESOLVED 2026-06-02` (PR #76 `feat/filter-gain-compensation`): three per-output makeup gains added (`lp/bp/hp → lpG/bpG/hpG`); LP/HP apply `filterGcMakeup(q)` (cancels the resonance peak; LP/HP `Q` is in dB so `Qlin = 10^(q/20)`), BP held at unity (0 dB-normalised). Validated to 0.00 dB residual vs the Chromium biquad math.
...
- **FilterE-F5 — GC behaviour not implemented (same as FilterC-F5).** Impl `setParam` FilterE branch doesn't consume `gainComp`. Severity: Critical. Disposition: `fix-toward-spec — RESOLVED 2026-06-02` (PR #76): makeup applied at the downstream `_output` gain, selected by `filterType` (LP/HP only), and squared for the 24 dB two-biquad cascade.
...
- **XFade-F5 — `FadeMod` writes to internal `fadeMod` gain that is never connected to the gainA/gainB pair. RESOLVED (2026-06-02).** The `fadeMod` GainNode was created (gain 0), accepted as the `FadeMod` input target, but never wired downstream, so connected modulation had no audible effect. Severity: Critical (the labelled mod input didn't function). **Fix (`_createXFade`, `src/AudioEngine.js`):** the inverse-gain-pair option from the original disposition. `fadeMod.gain` is set to unity so the CV passes through, then fanned out to the gain pair the same way the fade knob drives them (`setParam`: `gainA = 1 − fade`, `gainB = fade`) — `fadeMod → gainB.gain` (+) and `fadeMod → (−1 inverter) → gainA.gain` (−) — so a CV sweeps the crossfade bipolarly around the knob centre. No worklet/scheduler; no `MODULE_DEFS`, port, or param change; the inverter is added to `_nodes` for `removeModule` symmetry. Verified in Chromium Web Audio: an OfflineAudioContext render of the exact node graph shows the pre-fix topology inert (output 0.5 for FadeMod ∈ {0, ±0.4}) and the post-fix topology tracking the CV (0.5 → 0.1 / 0.9); and live in the running app (`examples/xfade-fademod-test.json`, LFO → FadeMod) the XFade output RMS sweeps 0.01–0.44 at the LFO rate. The Type I input attenuator (XFade-F3) stays folded to S2 — out of scope here. Disposition: `fix-toward-spec` (applied).
...
- Dispositions: 1 `fix-toward-spec` applied (F5 — critical, mod-input dead-wire repaired 2026-06-02).
- Code change applied: `src/AudioEngine.js` `_createXFade` — `FadeMod` fanned out to the gainA/gainB pair via a −1 inverter (inverse-gain-pair option); `fadeMod.gain` 0 → 1; inverter added to `_nodes`.
- Patch-load impact: none (internal rewiring only — no param rename, no range change, no `MODULE_DEFS`/port change; existing XFade patches load unchanged and FadeMod now functions).
...
- **FilterC-F5 / FilterE-F5**: ~~Gain Compensation params exist but the behaviour is never applied — dead controls.~~ **RESOLVED 2026-06-02 (PR #76)** — GC now cancels the resonance peak boost (per-type, dB-aware makeup; BP/notch untouched).
